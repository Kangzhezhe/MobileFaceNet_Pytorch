...
from config import CASIA_DATA_DIR, LFW_DATA_DIR, MY_DATA_DIR
from core import model
from core.utils import init_log
from dataloader.CASIA_Face_loader import CASIA_Face
from dataloader.CASIA_Face_loader_enhanced import CASIA_Face_Enhanced
from dataloader.LFW_loader import LFW
from dataloader.MY_Face_loader import MY_Face
...
# trainset_casia = CASIA_Face(root=CASIA_DATA_DIR)
# trainloader = torch.utils.data.DataLoader(trainset, batch_size=BATCH_SIZE,
#                                           shuffle=True, num_workers=8, drop_last=False)

trainset_casia_enhanced = CASIA_Face_Enhanced(root=CASIA_DATA_DIR)

trainset_my = MY_Face(root=MY_DATA_DIR)
# trainloader = torch.utils.data.DataLoader(trainset, batch_size=128,
#                                           shuffle=True, num_workers=4, drop_last=False)

trainset = torch.utils.data.ConcatDataset([trainset_my,trainset_casia_enhanced])
...
ArcMargin = model.ArcMarginProduct(128, trainset_my.class_nums+trainset_casia_enhanced.class_nums)
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, TOTAL_EPOCH+1):
        ...
        for data in tqdm(trainloader):
            ...
            total += batch_size
            # _print('training: {} / {}'.format(total, len(trainset)))
